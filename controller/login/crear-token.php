<?php
    ...
    if (!empty($_POST["token"]) && !empty($_POST["correo"]) && !empty($_POST["id"])){
        ...
        if(!$result){
            ...
            $message = '<html><body>';
            //Cabecera
            $message .= '<header><h1>Mensaje de recuperaci&oacute;n de contrase&ntilde;a</h1></header>';
            //cuerpo del mensaje
            $message .= '<main><div><p>Saludos estimado(a), mediante el presente mensaje le informamos ';
            $message .= 'que su solicitud de cambio de contrase&ntilde;a a sido aceptada. A continuaci&oacute;n haga click en el enlace ';
            $message .= 'para redirigirlo para que pueda cambiar su contrase&ntilde;a.</p><br>';
            //link
